    ...
    public class recvdatathread
    {
        ...
        protected void RecvDataFunc()
        {
            ...
            while (m_bStart)
            {
                lock (locker)
                {
                    len = ZlgcanMethod.ZCAN_GetReceiveNum(channel_handle_, TYPE_CAN);
                    if (len > 0)
                    {
                        int size = Marshal.SizeOf(typeof(ZCAN_Receive_Data));
                        IntPtr ptr = Marshal.AllocHGlobal((int)len * size);
                        len = ZlgcanMethod.ZCAN_Receive(channel_handle_, ptr, len, 50);
                        for (int i = 0; i < len; ++i)
                        {
                    ...
                    len = ZlgcanMethod.ZCAN_GetReceiveNum(channel_handle_, TYPE_CANFD);
                    if (len > 0)
                    {
                        int size = Marshal.SizeOf(typeof(ZCAN_ReceiveFD_Data));
                        IntPtr ptr = Marshal.AllocHGlobal((int)len * size);
                        len = ZlgcanMethod.ZCAN_ReceiveFD(channel_handle_, ptr, len, 50);
                        for (int i = 0; i < len; ++i)
                        {
