...
{
    /// <summary>
    /// CANFrameDataConfigWnd.xaml 的交互逻辑
    /// Can帧数据配置界面
    /// </summary>
    public partial class CANFrameDataConfigWnd : Window
    {
       ...
       /// <param name="e"></param>
        ...
        private void Type_ComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            CANFrameDataConfigVm vm = DataContext as CANFrameDataConfigVm;
            if (vm != null && cb.SelectedItem != null)
            {
                int lastIndex;
                if (vm.IsTypeOverflow(cb.SelectedItem.ToString(), out lastIndex))
                {
                    cb.SelectedIndex = lastIndex;
                }
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void Type_ComboBox_GotMouseCapture(object sender, MouseEventArgs e)
        {
            ComboBox cb = sender as ComboBox;
            CANFrameDataConfigVm vm = DataContext as CANFrameDataConfigVm;
            if (vm != null)
            {
                vm.UpdateLastTypeIndex(cb.SelectedIndex);
            }
        }
